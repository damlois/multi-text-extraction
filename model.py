...
async def async_extract_text_from_image(image_url: str) -> str:
    loop = asyncio.get_event_loop()
    extracted_text = await loop.run_in_executor(None, extract_text_from_image, image_url)
    return extracted_text


async def process_images(image_urls):
    tasks = [async_extract_text_from_image(image_url) for image_url in image_urls]
